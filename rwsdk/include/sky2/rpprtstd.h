 ...
 * The rotation is specified in radians. Multiple revolutions can be obtained by specifying an end rotation > PI.
 *
...
/*---- start: c:/daily/rwsdk/plugin/prtstd/sky2/prtstdplatform.h----*/

#define SKY_USE_STOP_FLAGx
#define SKY_USE_MULTIBUFERING

/**
 * \defgroup rpprtstdps2 PlayStation 2
 * \ingroup rpprtstd
 *
...
/**
 * \defgroup rpprtstdps2usage PlayStation 2 Hardware Usage
 * \ingroup rpprtstdps2
 *
 * \par ScratchPad & VU0 co-processor.
 * A set of PlayStation 2 specific functions are provided as replacement to the
 * standard generic C functions. These functions make uses of the PlayStation 2
 * hardwares to provide better performance than the generic C functions. These
 * function uses the PlayStation 2's ScratchPad memory and the VU0 co-processor.
 *
 * \par 
 * Care must be taken when using these functions to ensure they do not conflict
 * with any application's data and code that also uses these resources.
 * 
 * \par Multi-Buffering
 * To avoid timelapse glitches, \ref rpprtstd uses a multi-buffered PTank 
 * (\ref rpPTANKSKYFLAGINSTANCEBUFFER). While removing any potential visible 
 * problems this flag doubles the PTank memory footprint.
 * \par 
 * If an application is well scheduled, or if the particle emitter settings 
 * allow it, this behavior can be canceled by creating a custom, non 
 * multi-buffered, PTank.
 * \par 
 * The pClone example show how to create a custom PTank object.

 * \par Maximum Batch Size
 * The maximum batch size for an emitter depends on the properties carried by 
 * the particles. When creating new particle batches, \ref rpprtstd 
 * automatically checks and updates that value to the optimal one if needed. 
 * This will limit the number of new particles emitted per emission by the 
 * same value. Whenever particles and emitters are streamed out, the emitter 
 * will hold the optimized value, overwriting any existing settings.
 */
...
extern RpPrtStdParticleBatch *
RpPrtStdParticleStdUpdateSkyCB(RpPrtStdEmitter *emt,
                               RpPrtStdParticleBatch *prtBatch,
                               void *data);

extern RpPrtStdEmitter *
RpPrtStdEmitterStdEmitSkyCB(RpAtomic *atomic,
                            RpPrtStdEmitter *emt,
                            void *data);

extern RpPrtStdEmitterClass *
RpPrtStdEClassStdSetupSkyCB(RpPrtStdEmitterClass *eClass);

extern RpPrtStdParticleClass *
RpPrtStdPClassStdSetupSkyCB(RpPrtStdParticleClass *pClass);

extern RpPrtStdParticleClass *
RpPrtStdPClassStdCreateSky( RwInt32 dataFlag );

extern RpPrtStdEmitterClass *
RpPrtStdEClassStdCreateSky( RwInt32 dataFlag );

extern RwInt32
_rpPrtStdParticleStdGetBatchSizeSky( RpPrtStdEmitter *emt );
...
/*---- end: c:/daily/rwsdk/plugin/prtstd/sky2/prtstdplatform.h----*/
