...
void clearGsMem(int r, int g, int b, int disp_width, int disp_height)
{
  ...
  // setting GIF tag for scedb.draw0 structure
  SCE_GIF_CLEAR_TAG(&scedb.giftag0);
    __asm__ __volatile__ ("sync.l"); // wait until giftag is filled with 0
  scedb.giftag0.NLOOP = 8;
  scedb.giftag0.EOP = 1;
  scedb.giftag0.NREG = 1;
  scedb.giftag0.REGS0 = 0xe; // A_D
  ...
  sceGsSetDefDrawEnv(&scedb.draw0, SCE_GS_PSMCT32, disp_width,
		     bound(disp_height/2, 32)*2 + bound(disp_height, 32)*2,
		     0, 0);
  *(u_long *)&scedb.draw0.xyoffset1 = SCE_GS_SET_XYOFFSET_1(0, 0);

  FlushCache(0);
  sceGsSyncPath(0, 0);
  sceGsPutDrawEnv(&scedb.giftag0);
...
int vblankHandler(int val)
{
  ...
  if (isCountVblank) {
	...
	return 0;
      ...
      sceGsSetHalfOffset((oddeven&1)?(sceGsDrawEnv1*)UncAddr(&scedb.draw1)
			 :(sceGsDrawEnv1*)UncAddr(&scedb.draw0),
			 2048, 2048, oddeven^0x1);
      
      if ((oddeven == 0) && (tag->status == VOBUF_STATUS_FULL)) {
	sceGsSwapDBuff(&scedb, 0);
