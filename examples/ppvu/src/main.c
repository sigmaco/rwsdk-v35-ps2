 ...
 */
static RpAtomic *
AtomicCB(RpAtomic *a, void *data __RWUNUSED__)
{
    return CustomFXAtomicSetup(a);
}
...
CreateClump(void)
{
    ...
    static const RwV3d pos   = {0.0f, 0.0f, 10.0f};
    RwFrame *clumpFrame = NULL;
    RpClump *clump = NULL;
    RwTexture *customTexture = NULL;
    RwChar *path;
    ...
    customTexture = RwTextureRead(RWSTRING("rw"),NULL);

    CustomFXSetTexture(customTexture);
     ...
     * Setup CustomFX...
     */
    RpClumpForAllAtomics(clump, AtomicCB, NULL);
...
CreateAmbientLight(RpWorld *world)
{
    ...
    if( light )
    {
        RwRGBAReal AmbientLightColor = { 0.5f, 0.5f, 0.5f, 1.0f };

        RpLightSetColor(light,&AmbientLightColor);
...
Initialize(void)
{
    if( RsInitialize() )
    {
        ...
        RsGlobal.appName = RWSTRING("RenderWare Graphics PowerPipe VU Example");
...
Initialize3D(void *param)
{
    ...
    {
        RsErrorMessage(RWSTRING("Cannot create Ambient light sources"));
    }

    /*
     * Create CustomFX pipeline...
     */
    if( !CustomFXCreate() )
    {
        RsErrorMessage(RWSTRING("Cannot create CustomFX pipeline"));
    }
...
Idle(void)
{
    ...
    /*
     * Update texture Datas...
     */
    cumulDelta += deltaTime;

    CustomFXSetAngle(&cumulDelta);

    setupVector.x = RwCos(cumulDelta/2.0f);
    setupVector.y = RwSin(cumulDelta/2.0f);
    CustomFXSetTranslation(&setupVector);

    setupVector.x = RwCos(cumulDelta/8.0f)*4.0f;
    setupVector.y = RwCos(cumulDelta/8.0f)*4.0f;
    CustomFXSetScale(&setupVector);
