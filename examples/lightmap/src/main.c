...
FileLoad(RwChar *filename)
{
    ...
    if( rwstrstr(path, RWSTRING(".dff")) || rwstrstr(path, RWSTRING(".DFF")) )
    {
        ...
        if(NULL != newClump)
        {
            ...
            if (RpLtMapWorldLightMapsQuery(World) > 0)
            {
                /* Atomics in the loaded clump may contain lightmaps */
...
wireframeSectorCB(RpWorldSector *sector)
{
    ...
    numTriangles = sector->numPolygons;
    if ((1) &&
        (NOWIREFRAME != gDrawWireframe) )
    {
        ...
        {
            indices[j++] = sector->polygons[i].vertIndex[0];
            indices[j++] = sector->polygons[i].vertIndex[1];
            indices[j++] = sector->polygons[i].vertIndex[1];
            indices[j++] = sector->polygons[i].vertIndex[2];
            indices[j++] = sector->polygons[i].vertIndex[2];
            indices[j++] = sector->polygons[i].vertIndex[0];
        }
        for (i = 0;i < 24;i++)
        ...
        (NULL != sector->normals) )
    {
        ...
        vertices = (RwIm3DVertex *)
            RwMalloc(2*numVertices*sizeof(RwIm3DVertex));
        RSASSERT(NULL != vertices);
