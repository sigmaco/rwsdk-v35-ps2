...
RwBool   gCreatedLightMaps  = FALSE;
...
ResetScene(RpWorld *world, RwCamera *camera)
{
    ...
    /* An empty world contains no lightmaps! */
    gCreatedLightMaps = FALSE;
 ...
 */
static RpMaterial *
AtomicMaterialCB(RpMaterial *mat, void *data __RWUNUSED__)
{
    RtLtMapMaterialSetLightMapDensityModifier(mat, 2);
...
SetupExampleAtomicCB(RpAtomic *atomic, void *data)
{
    ...
    /* Increase lightmap sample density for atomics */
    RpGeometryForAllMaterials(geom, AtomicMaterialCB, NULL);
    ...
    {
        /* Enable vertex lighting (and turn off lightmapping)
         * for the first atomic in the scene */
        RtLtMapAtomicSetFlags(atomic, (RtLtMapAtomicGetFlags(atomic)
            | rtLTMAPOBJECTVERTEXLIGHT) & ~rtLTMAPOBJECTLIGHTMAP);
    }
...
DrawPointLight(RpLight *light, RwCamera *camera)
{
    ...
    for(i = 0;i < POINT_LIGHT_NUM_VERTICES;i++)
    {
        ...
        RwV3dTransformPoints(&point, &point, 1, alignMatrix);
...
saveImageCB(RwTexture *texture, void *data)
{
    ...
    fullPath = (RwChar *)RwMalloc(pathLength*sizeof(RwChar));
 ...
 */
static RpAtomic *
AtomicPreLightClearCB(RpAtomic *atomic, void *data)
{
    RwBool clearVertexLitObjects = *(RwBool *)data;
    RpGeometry *geom = RpAtomicGetGeometry(atomic);
 ...
 */
static RpClump *
ClumpPreLightClearCB(RpClump *clump, void *data)
{
    RpClumpForAllAtomics(clump, AtomicPreLightClearCB, data);
    return(clump);
}
...
_loadWorld(RwChar *file)
{
    ...
    if (NULL != file)
    {
       ...
       *search = '\0';
        ...
        /* This is "[path]/[filename]/[filename].txd" */
        rwstrcat(string1, string2);
        rwstrcat(string1, gPlatformString);
        rwstrcat(string1, ".txd");

        /* Create an appropriate path for a texture dictionary */
        texDictPath = RsPathnameCreate(string1);

        /* This is "[path]/[filename]/[filename][target]lm.txd" */
        rwstrcpy(string1, imagePath);
        rwstrcat(string1, string2);
        rwstrcat(string1, gPlatformString);
        rwstrcat(string1, RWSTRING("lm.txd"));

        /* Create an appropriate path for a lightmap texture dictionary */
        lmTexDictPath = RsPathnameCreate(string1);

        /* This is "[path]/[filename]/[filename]pilm.txd" */
        rwstrcpy(string1, imagePath);
        rwstrcat(string1, string2);
        rwstrcat(string1, RWSTRING("pilm.txd"));

        /* Create an appropriate path for a lightmap PI texture dictionary */
        lmPITexDictPath = RsPathnameCreate(string1);

        /* This is "[path]/[filename]/[filename].dff" */
        rwstrcpy(string1, imagePath);
        rwstrcat(string1, string2);
        rwstrcat(string1, RWSTRING(".dff"));

        /* Create an appropriate path for DFF containing lights */
        lightsDffPath = RsPathnameCreate(string1);
    }
    else
    {
        return(NULL);
...
LoadWorld(RwChar *file, RpWorld *oldWorld, RwCamera *camera)
{
    ...
    if (RpLtMapWorldLightMapsQuery(newWorld) > 0)
    {
        /* Do lightmaps exist for the loaded objects? */
        gCreatedLightMaps = TRUE;
    }
    ...
    /* Clear prelights to white (for non-lightmapped, non-vertex-lit objects)
    ...
    RpWorldForAllClumps(
        newWorld, ClumpPreLightClearCB,  &clearVertexLitObjects);
...
{
    if ((FALSE != gLightingWorld) || (FALSE != gResetLighting))
    {
        static RwUInt32  checkTotalSamples = 0;
        static RwCamera *lightingCamera    = (RwCamera *)NULL;
        RwInt32  deltaSamples;
        RwUInt32 i;

        if (FALSE != gResetLighting)
        {
            /* Someone loaded a new world or something */
            gLightingSession.startSample = 0;
            checkTotalSamples = 0;
        ...
        {
            if (0 == gLightingSession.startSample)
            {
                ...
                (FALSE != gUseAreaLights))
            {
                ...
                RtLtMapSetAreaLightRadiusModifier(0.5f);
            ...
            /* gLightingSession.startSample persists from the
             * last slice and we process some more samples */
            gLightingSession.numSamples = SAMPLESPERSLICE;
            ...
            {
                deltaSamples = RtLtMapIlluminate(&gLightingSession, gAreaLights,
                    (RwUInt32) LightMapScale);
            }
            else
            {
                deltaSamples = RtLtMapIlluminate(
                   &gLightingSession, (RtLtMapAreaLightGroup *)NULL,
                    (RwUInt32) LightMapScale);
            }
            RSASSERT(-1 != deltaSamples);
            ...
            if (0 == gLightingSession.startSample)
            {
                checkTotalSamples = gLightingSession.totalSamples;
            }
            else
            {
                /* Ensure nothing's changed that should have reset lighting */
                RSASSERT(checkTotalSamples == gLightingSession.totalSamples);
            }

            /* deltaSamples won't necessarily be equal to SAMPLESPERSLICE,
             * because incremental lighting is quantized to be per-polygon. */
            gLightingSession.startSample += deltaSamples;

            gLightingProgress = gLightingSession.startSample /
                (RwReal)gLightingSession.totalSamples;
...
{
    RwReal        length, density;
    const RwBBox *box;
    RwV3d         size;

    if (FALSE != gCreatingLightMaps)
    {
        RSASSERT(NULL != gLightingSession.world);
        box = RpWorldGetBBox(gLightingSession.world);
        ...
        density = 500.0f / length;
        density *= LightMapDensity;
        RtLtMapLightMapSetDefaultSize(LightMapSize);

        /* NOTE: this will destroy any existing lightmaps */
        RtLtMapLightMapsCreate(&gLightingSession,
            density, (RwRGBA *)NULL);
...
ClearLightMaps(void)
{
    if (FALSE != gClearingLightMaps)
    {
        ...
        else
        {
             ...
             * or white (for lightmapped objects) */
            RSASSERT(NULL != gLightingSession.world);
...
DestroyLightMaps(void)
{
    ...
    RpWorldForAllClumps(
        gLightingSession.world, ClumpPreLightClearCB,  &clearVertexLitObjects);
