...
Initialize(void)
{
    if (RsInitialize())
    {
        ...
        RsGlobal.appName = RWSTRING("RenderWare Graphics PS2All Custom Pipeline Example");
...
{
    /* Because we're drawing so many atomics per frame, we need to enlarge the DMA buffers */
    _rwDMAPreAlloc(3*1024*1024, 0, NULL);
