...
/* */
int
audioDecCreate(AudioDec * ad,
               u_char * buff, int buffSize, int iopBuffSize)
{
    void *addr;
    ...
    /* */
    addr = sceSifAllocIopHeap(iopBuffSize);
    ad->iopBuff = (int) addr;
    if (ad->iopBuff < 0)
    {
    ...
    /* */
    addr = sceSifAllocIopHeap(ZERO_BUFF_SIZE);
...
/* */
void
audioDecEndPut(AudioDec * ad, int size)
{
    if (ad->state == AU_STATE_INIT)
    {
        const int           compare = AU_HDR_SIZE - ad->hdrCount;
        int                 hdr_add = min(size, compare);
...
/* */
static void
iopGetArea(int *pd0, int *d0, int *pd1, int *d1, AudioDec * ad, int pos)
{
    int                 len =
        (pos + ad->iopBuffSize - ad->iopLastPos -
         UNIT_SIZE) % ad->iopBuffSize;
...
/* */
static int
sendToIOP2area(int pd0, int d0, int pd1, int d1,
...
/* */
static int
sendToIOP(int dst, u_char * src, int size)
{
