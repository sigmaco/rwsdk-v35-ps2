...
int                 mpegError(sceMpeg * mp,
                              sceMpegCbDataError * cberror,
                              void *anyData);
int                 mpegNodata(sceMpeg * mp, sceMpegCbData * cbdata,
                               void *anyData);
int                 mpegStopDMA(sceMpeg * mp, sceMpegCbData * cbdata,
                                void *anyData);
int                 mpegRestartDMA(sceMpeg * mp, sceMpegCbData * cbdata,
                                   void *anyData);
int                 mpegTS(sceMpeg * mp, sceMpegCbDataTimeStamp * cbts,
                           void *anyData);
static int          cpy2area(u_char * pd0, int d0, u_char * pd1, int d1,
                             u_char * ps0, int s0, u_char * ps1,
                             int s1);
               ...
               int tagSize, TimeStamp * pts, int n_pts)
{
    ...
    sceMpegAddCallback(&vd->mpeg,
                       sceMpegCbError, (sceMpegCallback) mpegError,
                       NULL);
...
/* */
void
videoDecSetDecodeMode(VideoDec * vd, int ni, int np, int nb)
{
...
/* */
int
videoDecSetStream(VideoDec * vd, int strType, int ch,
...
/* */
void
videoDecEndPut(VideoDec * vd, int size)
{
...
/* */
u_int
videoDecSetState(VideoDec * vd, u_int state)
{
    u_int               old = vd->state;
...
/* */
int
videoDecPutTs(VideoDec * vd, long pts_val,
...
/* */
int
videoDecInputCount(VideoDec * vd)
{
...
/* */
int
videoDecInputSpaceCount(VideoDec * vd)
{
    u_char             *ptr0;
    u_char             *ptr1;
    int                 len0, len1;
...
decBs0(VideoDec * vd)
{
    ...
    while (!sceMpegIsEnd(&vd->mpeg))
    {
        ...
        /* */
        ret =
            sceMpegGetPicture(mp, rgb32,
                              MAX_WIDTH / 16 * MAX_HEIGHT / 16);

        if (ret < 0)
        {
            static char         _sceMpegGetPicture_decode_error[] =
                "sceMpegGetPicture() decode error\n";

            ErrMessage(_sceMpegGetPicture_decode_error);
        }
...
/* */
int
mpegError(sceMpeg * mp __UNUSED__,
          sceMpegCbDataError * cberror, void *anyData __UNUSED__)
{
    printf("%s\n", cberror->errMessage);
    return 1;
}
...
/* */
int
mpegNodata(sceMpeg * mp __UNUSED__,
           sceMpegCbData * cbdata __UNUSED__, void *anyData __UNUSED__)
{
    switchThread();
...
/* */
int
mpegStopDMA(sceMpeg * mp __UNUSED__,
            sceMpegCbData * cbdata __UNUSED__, void *anyData __UNUSED__)
{
    /* Stop DMA */
...
/* */
int
mpegRestartDMA(sceMpeg * mp __UNUSED__,
               sceMpegCbData * cbdata __UNUSED__,
               void *anyData __UNUSED__)
{
    /* Restart DMA */
...
/* */
int
mpegTS(sceMpeg * mp __UNUSED__,
       sceMpegCbDataTimeStamp * cbts, void *anyData __UNUSED__)
{
    TimeStamp           ts;
...
/* */
static int
cpy2area(u_char * pd0, int d0, u_char * pd1, int d1,
