...
inline int
IsPtsInRegion(int tgt, int pos, int len, int size)
{
...
int
getFIFOindex(ViBuf * f, void *addr)
{
...
void
scTag2(QWORD * q, void *addr, u_int id, u_int qwc)
{
    q->l[0] =
        (u_long) (u_int) addr << 32
        | (u_long) id << 28 | (u_long) qwc << 0;
}
...
viBufAddDMA(ViBuf * f)
{
    ...
    if (!f->isActive)
    {
        static char         _DMA_ADD_not_active[] =
            "DMA ADD not active\n";

        ErrMessage(_DMA_ADD_not_active);
        return FALSE;
    }
...
/* */
int
viBufModifyPts(ViBuf * f, TimeStamp * new_ts)
{
    TimeStamp          *ts;
    int                 rd =
        (f->wt_ts - f->count_ts + f->n_ts) % f->n_ts;
    ...
    if (f->count_ts > 0)
    {
        while (loop)
        {
            ...
            if (IsPtsInRegion
                (ts->pos, new_ts->pos, new_ts->len, datasize))
            {
                int                 len =
                    min(new_ts->pos + new_ts->len - ts->pos, ts->len);
...
/* */
int
viBufPutTs(ViBuf * f, TimeStamp * ts)
{
    int                 ret = 0;
...
/* */
int
viBufGetTs(ViBuf * f, TimeStamp * ts)
{
