...
/* changed from db to mpegdb to avoid conflict with RW */
extern sceGsDBuff   mpegdb;
...
/* */
void
clearGsMem(int r, int g, int b, int disp_width, int disp_height)
{
    const u_long        giftag_clear[2] =
        { SCE_GIF_SET_TAG(0, 1, 0, 0, 0, 1),
        0x000000000000000eL
    };
    sceGifPacket        packet;
    u_long128          *packetBase =
        (u_long128 *) memalign(16, sizeof(u_long128) * 6);

    sceDmaChan         *dmaGif = sceDmaGetChan(SCE_DMA_GIF);

    /* setting GIF tag for mpegdb.draw0 structure */
    SCE_GIF_CLEAR_TAG(&mpegdb.giftag0);
    mpegdb.giftag0.NLOOP = 8;
    mpegdb.giftag0.EOP = 1;
    mpegdb.giftag0.NREG = 1;
    mpegdb.giftag0.REGS0 = 0xe; /* A_D */
    ...
    sceGsSetDefDrawEnv(&mpegdb.draw0, SCE_GS_PSMCT32, disp_width,
                       bound(disp_height / 2,
                             32) * 2 + bound(disp_height, 32) * 2, 0,
                       0);
    *(u_long *) & mpegdb.draw0.xyoffset1 = SCE_GS_SET_XYOFFSET_1(0, 0);

    FlushCache(0);
    sceGsSyncPath(0, 0);
    sceGsPutDrawEnv(&mpegdb.giftag0);
    ...
    /* packet for a big sprite polygon */
    {
        ...
        sceGifPkAddGsAD(&packet, SCE_GS_RGBAQ,
                        SCE_GS_SET_RGBAQ(r, g, b, 0, 0));
        sceGifPkAddGsAD(&packet, SCE_GS_XYZ2,
                        SCE_GS_SET_XYZ2(0 << 4, 0 << 4, 0));
        sceGifPkAddGsAD(&packet, SCE_GS_XYZ2,
                        SCE_GS_SET_XYZ2(MAX_WIDTH << 4,
                                        MAX_HEIGHT * 5 << 4, 0));
...
/* */
void
setImageTag(u_int * tags, void *image, int index, int image_w,
            int image_h)
{
    u_int               dbp, dbw, dpsm;
    u_int               dir, dsax, dsay;
    ...
    if (index == 0)
    {                          /* set image data to packet */

        /*  set params */
        dbp =
            (bound(MAX_WIDTH, 64) * bound((MAX_HEIGHT / 2), 32) * 2) /
            64;
        ...
        /* eop == 0 */
        {
            ...
            sceGifPkAddGsAD(&packet, SCE_GS_BITBLTBUF,
                            SCE_GS_SET_BITBLTBUF(0, 0, 0, dbp, dbw,
                                                 dpsm));
            sceGifPkAddGsAD(&packet, SCE_GS_TRXREG,
                            SCE_GS_SET_TRXREG(rrw, rrh));
            sceGifPkCloseGifTag(&packet);
        }
        ...
        for (i = 0; i < mbx; i++)
        {
            for (j = 0; j < mby; j++)
            {
                ...
                /* eop == 0 */
                {
                    sceGifPkOpenGifTag(&packet,
                                       *(const u_long128 *) giftag);
                    sceGifPkAddGsAD(&packet, SCE_GS_TRXPOS,
                                    SCE_GS_SET_TRXPOS(0, 0,
                                                      16 * i + dsax,
                                                      16 * j + dsay,
                                                      0));
                    sceGifPkAddGsAD(&packet, SCE_GS_TRXDIR,
                                    SCE_GS_SET_TRXDIR(xdir));
                    sceGifPkCloseGifTag(&packet);
                }

                {
                    u_long             *const tag =
                        (u_long *) sceGifPkReserve(&packet, 4);
                    tag[0] =
                        SCE_GIF_SET_TAG(16 * 16 * 4 / 16, 0, 0, 0, 2,
                                        0);
                    tag[1] = 0;
                }
                sceGifPkRef(&packet, DmaAddr(image), 16 * 16 * 4 / 16,
                            0, 0, 0);
    ...
    /* eop == 1 */
    {
        sceGifPkOpenGifTag(&packet, *(const u_long128 *) giftag_eop);
        ...
        sceGifPkAddGsAD(&packet, SCE_GS_TEX0_1,
                        SCE_GS_SET_TEX0_1((bound(MAX_WIDTH, 64)
                                          ...
                                          SCE_GS_PSMCT32, 10, 10, 0, 1,
                                          0, 0, 0, 0, 0));
        sceGifPkAddGsAD(&packet, SCE_GS_PRIM,
                        SCE_GS_SET_PRIM(6, 0, 1, 0, 0, 0, 1, 0, 0));
        sceGifPkAddGsAD(&packet, SCE_GS_UV,
                        SCE_GS_SET_UV(tex.x, tex.y));
        sceGifPkAddGsAD(&packet, SCE_GS_XYZ2,
                        SCE_GS_SET_XYZ2(poly.x, poly.y, 0));
        sceGifPkAddGsAD(&packet, SCE_GS_UV,
                        SCE_GS_SET_UV(tex.x + tex.w, tex.y + tex.h));
        sceGifPkAddGsAD(&packet, SCE_GS_XYZ2,
                        SCE_GS_SET_XYZ2(poly.x + poly.w,
                                        poly.y + poly.h, 0));
        sceGifPkCloseGifTag(&packet);
    }
...
/* */
int
vblankHandler(int __UNUSED__ val)
{
    ...
    if (isCountVblank)
    {
        ...
        {                      /* no error */
            ...
            sceGsSetHalfOffset((oddeven & 1) ? (sceGsDrawEnv1 *)
                               UncAddr(&mpegdb.
                                       draw1) : (sceGsDrawEnv1 *)
                               UncAddr(&mpegdb.draw0), 2048, 2048,
                               oddeven ^ 0x1);

            if ((oddeven == 0) && (tag->status == VOBUF_STATUS_FULL))
            {
                sceGsSwapDBuff(&mpegdb, 0);

                /* Load image data to GS using path3 */
                sceGsSyncPath(0, 0);
                sceDmaSend(dmaGif_loadimage,
                           (u_long128 *) ((u_int) tag->v[0]));
                ...
                tag->status = VOBUF_STATUS_TOPDONE;
                     ...
                     && tag->status == VOBUF_STATUS_TOPDONE)
            {
                sceGsSwapDBuff(&mpegdb, 1);

                /* Load image data to GS using path3 */
                sceGsSyncPath(0, 0);
                sceDmaSend(dmaGif_loadimage,
                           (u_long128 *) ((u_int) tag->v[1]));
...
/*  */
int
handler_endimage(int __UNUSED__ val )
{
    if (isFrameEnd)
    {
        voBufDecCount(&voBuf);
